...
class UserService {
  ...
  static async verifyEmail(email, locale) {
    ...
    if (prev) {
      ...
      let result = await EmailVerifyModel.create({
        email: email,
        verify_code: verificationCode,
      });
      if (!result) {
  ...
  static async getUsers(sortBy, pageNumber, itemsPerPage) {
    ...
    userList = userList.map((user) => {
      user.have_wallet === 1
        ? (user.have_wallet = true)
        : (user.have_wallet = false);
      return user;
    });
    let userData = userList.slice(
    ...
    return {
      response: true,
      message: "Success",
      data: { totalNumber: userList.length, users: userData },
    };
  }
  ...
  static async signUp(rawData) {
    ...
    if (parseInt(rawData.invite_code) !== 0) {
      let inviter = await UserModel.findOne({
        id: parseInt(rawData.invite_code),
      });
      if (!inviter) {
  ...
  static async userLogin(rawData) {
    ...
    if (wallets.length === 0) {
      return {
        response: true,
        message: "Success!",
        data: { userInfo, token, keyPair: null },
      };
    }
    let keyPair = [];
    ...
    return {
      response: true,
      message: "Success!",
      data: { userInfo, token, keyPair: keyPair },
    };
  }
