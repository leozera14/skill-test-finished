...
class WalletService {
  ...
  static async createWallet(userId, keyphrase, locale) {
    ...
    await ManageUserModel.update(
      { have_wallet: true, wallet_address: wallet.publickey },
      userId
    );

    const subject = i18n.__({
      phrase: "MGL Exchange - Wallet Created Sucessfully",
      locale: locale,
    });
    const body = i18n.__({
      phrase:
        "Your MGL Exchange wallet has been created sucessfully. Thank you.",
      locale: locale,
    });
    ...
    return {
      response: true,
      message: "success",
      data: { publicKey: wallet.publickey, privateKey: privateKey },
    };
  }
  ...
  static async accessWalletWithPrivateKey(rawData, userId) {
    ...
    if (wallet.user_id !== userId) {
      return {
        response: false,
        message: "This is not your wallet.",
        data: null,
      };
    }
  ...
  static async addToken(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      ...
      result = await WalletModel.update(
        { polygon_tokensymbol: symbols, polygonmain_assets: assets },
        wallet.id
      );
    } else if (rawData.network === "polygon-testnet") {
      ...
      symbols = wallet.polygon_tokensymbol + "," + rawData.token_symbol;
      result = await WalletModel.update(
        { polygon_tokensymbol: symbols, polygontest_assets: assets },
        wallet.id
      );
    } else if (rawData.network === "bsc-mainnet") {
      ...
      result = await WalletModel.update(
        { bsc_tokensymbol: symbols, bscmain_assets: assets },
        wallet.id
      );
    } else if (rawData.network === "bsc-testnet") {
      ...
      symbols = wallet.bsc_tokensymbol + "," + rawData.token_symbol;
      result = await WalletModel.update(
        { bsc_tokensymbol: symbols, bsctest_assets: assets },
        wallet.id
      );
    }
    if (result === "") {
  ...
  static async getBalance(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      let result = await new Wallet(rawData.network).getBalance(
        wallet.publickey,
        wallet.polygon_tokensymbol,
        wallet.polygonmain_assets
      );
      ...
      let result = await new Wallet(rawData.network).getBalance(
        wallet.publickey,
        wallet.polygon_tokensymbol,
        wallet.polygontest_assets
      );
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "bsc-mainnet") {
      let result = await new Wallet(rawData.network).getBalance(
        wallet.publickey,
        wallet.bsc_tokensymbol,
        wallet.bscmain_assets
      );
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "bsc-testnet") {
      let result = await new Wallet(rawData.network).getBalance(
  ...
  static async getPrice(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      let result = await new Wallet(rawData.network).getTokenPrice(
        wallet.polygonmain_assets
      );
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "polygon-testnet") {
  ...
  static async getCurrentEthPrice(rawData, userId) {
    let wallet = await WalletModel.findOne({ publicKey: rawData.publicKey });
    ...
    const { price } = await TopTokenModel.findOne({ symbol: "ETH" });
  ...
  static async getAssets(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      ...
      result = wallet.polygonmain_assets.split(",");
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "polygon-testnet") {
      result = wallet.polygontest_assets.split(",");
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "bsc-mainnet") {
      result = wallet.bscmain_assets.split(",");
      return { response: true, message: "Success", data: result };
    } else if (rawData.network === "bsc-testnet") {
      result = wallet.bsctest_assets.split(",");
      return { response: true, message: "Success", data: result };
    }
  }
  ...
  static async sendToken(rawData, userId) {
    ...
    if (rawData.network === "polygon-mainnet") {
      if (!wallet.polygonmain_assets.includes(rawData.token)) {
        return {
          response: false,
          message: "You don't have that token in your wallet.",
          data: null,
        };
      }
      if (
        rawData.destination === "0x0000000000000000000000000000000000000000"
      ) {
      ...
      result = await new Wallet(rawData.network).sendToken(
        wallet.privatekey,
        wallet.publickey,
        rawData.destination,
        rawData.token,
        parseFloat(rawData.amount),
        parseInt(rawData.gasLimit),
        parseInt(rawData.gasPrice)
      );
    } else if (rawData.network === "polygon-testnet") {
      if (!wallet.polygontest_assets.includes(rawData.token)) {
        return {
    ...
    let status = await TransactionModel.create({
      user_id: userId,
      hash: result.hash,
      from_id: wallet.publickey,
      to_id: rawData.destination,
      token: rawData.token,
    ...
    if (!status) {
      return {
        response: true,
        message: "Token send Success, but transaction register failed.",
        data: result.hash,
      };
    }

    let user = await UserModel.findOne({ id: userId });
    const sendSubject = i18n.__({
      phrase: "MGL Exchange: Tokens sent",
      locale: rawData.locale || "En",
    });
    const sendBody = i18n.__(
      { phrase: "You have sent %s %s to %s", locale: rawData.locale || "En" },
      rawData.amount,
      rawData.token,
      rawData.destination
    );
    ...
    let receiver = await ManageUserModel.findOne({
      publickey: rawData.destination,
    });
    if (receiver) {
      const recvSubject = i18n.__({
        phrase: "MGL Exchange: Tokens received",
        locale: rawData.locale || "En",
      });
      const recvBody = i18n.__(
        {
          phrase: "You have received %s %s tokens in your wallet.",
          locale: rawData.locale || "En",
        },
        rawData.amount,
        rawData.token
  ...
  static async getTransaction(rawData, userId) {
    let transactionList = await TransactionModel.findMore({
      from_id: rawData.publicKey,
      to_id: rawData.publicKey,
      network: rawData.network,
    });
  ...
  static async receiveToken(rawData, userId) {
    ...
    let result = [];
    if (rawData.network === "polygon-mainnet") {
      result = new Wallet(rawData.network).receiveToken(
        rawData.balance,
        wallet.publickey,
        wallet.polygonmain_assets
      );
  ...
  static async getCoinForTransaction(rawData, userId) {
    ...
    if (receiver.get_bnb) {
      return {
        response: false,
        message: "You already received service Coin",
        data: null,
      };
    }
    let admin = await UserModel.findOne({ role: Role.Super });
    ...
    let bnbPrice = await new Wallet().getTokenPriceInUsd(
      rawData.network,
      BNB_ADDRESS
    );
    let amount = (COIN_AMOUNT / bnbPrice).toFixed(10);
    let result = await new Wallet(rawData.network).sendToken(
